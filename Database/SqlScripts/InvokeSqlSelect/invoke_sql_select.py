"""
SYNOPSIS
    Runs a SELECT statement against the tenant database and prints the results.
...
DESCRIPTION
    ...
    Python port of Invoke-SqlSelect.ps1.
...
def parse_args():
    ...
    parser.add_argument("--config-path", default=str(SCRIPT_FOLDER / "SqlSelectConfig.json"))
    parser.add_argument("--query-path", default=str(SCRIPT_FOLDER / "select_query.sql"))
...
def main():
    args = parse_args()
    logger, correlation_id = db_logging.build_logger(SCRIPT_FOLDER, "invoke_sql_select")
