"""
SYNOPSIS
    Runs an INSERT statement against the tenant database. Uses the sql_common
...
DESCRIPTION
    ...
    Python port of Invoke-SqlInsert.ps1.
...
def parse_args():
    ...
    parser.add_argument("--config-path", default=str(SCRIPT_FOLDER / "SqlInsertConfig.json"))
    parser.add_argument("--query-path", default=str(SCRIPT_FOLDER / "insert_query.sql"))
...
def main_process(args, logger):
    ...
    logger.info("Inserted %s row(s).", rows_affected)


def main():
    args = parse_args()
    logger, correlation_id = db_logging.build_logger(SCRIPT_FOLDER, "invoke_sql_insert")
