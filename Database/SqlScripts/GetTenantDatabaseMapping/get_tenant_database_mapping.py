"""
SYNOPSIS
    Lists each tenant with the database it's configured to use. Uses the
...
DESCRIPTION
    Joins [bsone_core].[dbo].[Tenant] to [bsone_core].[dbo].[Tenant.Credential] (on
    TenantID) to [bsone_core].[dbo].[Server.Instance.Database] (on DatabaseID) to
    show TenantID, CompanyName, and the resolved database Name from
    Server.Instance.Database. Connection details are read from --config-path; the
    SQL itself is read from --query-path, so editing that .sql file changes what
    gets run without touching this script.
    Python port of Get-TenantDatabaseMapping.ps1.
...
def parse_args():
    ...
    parser.add_argument("--config-path", default=str(SCRIPT_FOLDER / "SqlTenantDatabaseMappingConfig.json"))
    parser.add_argument("--query-path", default=str(SCRIPT_FOLDER / "tenant_database_mapping.sql"))
...
def main_process(args, logger):
    ...
    sql_common.print_table(columns, rows)
    logger.info("Returned %s row(s).", len(rows))


def main():
    args = parse_args()
    logger, correlation_id = db_logging.build_logger(SCRIPT_FOLDER, "get_tenant_database_mapping")
