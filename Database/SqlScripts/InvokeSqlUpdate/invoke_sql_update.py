"""
SYNOPSIS
    Runs an UPDATE statement against the tenant database. Uses the sql_common
...
DESCRIPTION
    ...
    Python port of Invoke-SqlUpdate.ps1.
...
def parse_args():
    ...
    parser.add_argument("--config-path", default=str(SCRIPT_FOLDER / "SqlUpdateConfig.json"))
    parser.add_argument("--query-path", default=str(SCRIPT_FOLDER / "update_query.sql"))
...
def main():
    args = parse_args()
    logger, correlation_id = db_logging.build_logger(SCRIPT_FOLDER, "invoke_sql_update")
