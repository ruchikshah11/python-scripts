...
def parse_args():
    ...
    parser.add_argument("--content-type-name", default=None, help="Name of one content type; omit for all")
...
def main_process(args, logger):
    ...
    if args.content_type_name:
        content_type = sharepoint.get_content_type(ctx, args.content_type_name)
        logger.info("Name: %s", content_type.properties.get("Name"))
        logger.info("Id: %s", content_type.properties.get("StringId"))
        logger.info("Group: %s", content_type.properties.get("Group"))
        logger.info("Description: %s", content_type.properties.get("Description"))
    else:
        content_types = sharepoint.get_content_types(ctx)
        logger.info("Found %s content type(s):", len(content_types))
        for content_type in content_types:
            logger.info("- %s (Group: %s)", content_type.properties.get("Name"), content_type.properties.get("Group"))


def main():
    args = parse_args()
    logger, correlation_id = sp_logging.build_logger(SCRIPT_FOLDER, "get_content_types")
