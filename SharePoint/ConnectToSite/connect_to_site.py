"""
SYNOPSIS
    Connects to a SharePoint site interactively (browser sign-in) and prints
    the site's Title and Url. Uses the sharepoint library (SharePoint/sharepoint) for
    the connection, and the shared sp_logging helper (SharePoint/common) for
    logging.
...
def main_process(args, logger):
    ...
    logger.info("Connected to: %s", web.properties["Title"])
    logger.info("Site URL: %s", web.properties["Url"])


def main():
    args = parse_args()
    logger, correlation_id = sp_logging.build_logger(SCRIPT_FOLDER, "connect_to_site")
